...
public class SwitchButton extends View implements Checkable {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int textColor = optColor(typedArray,
                                 R.styleable.SwitchButton_sb_text_color,
                                 Color.WHITE);//Color.WHITE;

        int textSize = typedArray.getDimensionPixelSize(R.styleable.SwitchButton_sb_text_size, 50);
        ...
        uncheckText = typedArray.getString(R.styleable.SwitchButton_sb_unchecked_text);
        checkedText = typedArray.getString(R.styleable.SwitchButton_sb_checked_text);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        baseline = height * 1.0f / 2 + borderWidth + distance;

        if (!TextUtils.isEmpty(uncheckText)) {
            uncheckTextWidth = textPaint.measureText(uncheckText);
        }
        if (!TextUtils.isEmpty(checkedText)) {
            checkedTextWidth = textPaint.measureText(checkedText);
        }

        textMinX = getResources().getDimensionPixelSize(R.dimen.x8);
        textMaxX = w - getResources().getDimensionPixelSize(R.dimen.x8) - checkedTextWidth;
    }
...
//        viewState.textX = textMaxX;
    ...
    private void drawText(Canvas canvas) {
        if (TextUtils.isEmpty(checkedText) && TextUtils.isEmpty(uncheckText)) {
            return;
        }
        if ((viewState.buttonX == buttonMaxX && isChecked) || (viewState.buttonX == buttonMinX && !isChecked)) {
            canvas.drawText(isChecked ? checkedText : uncheckText, isChecked ? textMinX : textMaxX, baseline, textPaint);
        }
    }
    ...
    public void setCheck(boolean checked, boolean useAni) {
        enableEffect = useAni;
        setChecked(checked);
    }
...
//            afterState.textX = textMaxX;
    ...
    private float uncheckTextWidth;
    private float checkedTextWidth;
...
//                        viewState.textX = afterState.textX
//                                - (afterState.textX - beforeState.textX) * value;
                    }
